...
func ParseCmdArgs() (cmdArgs CmdArgs) {
	...
	zflag.StringVar(&cmdArgs.basePath, "base-path", "/", "Base path to serve the application (example: /gallery)")
	zflag.Parse()

	// normalize basePath: ensure it starts with "/" and remove trailing slash (except for root "/")
	if cmdArgs.basePath == "" {
		cmdArgs.basePath = "/"
	} else {
		if !strings.HasPrefix(cmdArgs.basePath, "/") {
			cmdArgs.basePath = "/" + cmdArgs.basePath
		}
		if cmdArgs.basePath != "/" {
			cmdArgs.basePath = strings.TrimRight(cmdArgs.basePath, "/")
		}
	}
